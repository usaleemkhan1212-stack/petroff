...
import { HeroOrnaments } from "@/components/sections/litiges/HeroOrnaments";
import { ConsultButton } from "@/components/consultation/ConsultButton";
import { Button } from "@/components/ui/Button";
import { Container } from "@/components/ui/Container";
import { MaybeLink } from "@/components/ui/MaybeLink";
import { cn } from "@/lib/utils";
import { heroStats } from "@/lib/litiges";

/**
 * Figma's `13986:934` - the Litiges entre associés hero.
 *
 * Two bands, like every sibling domain page: a 64px lilas breadcrumb, then a
 * 720px stage carrying eight ornaments and one centred 860 column pinned at a
 * literal `top: 64` - hence `lg:pt-16`.
 *
 * **Its title and lead carry their own measures inside that column** - 746 and
 * 712 against the family's full-width title and 640 lead - so both are capped
 * here rather than inherited.
 */
export function Hero() {
  const t = useTranslations("LitigesPage.hero");
      ...
      <Container className="relative">
        ...
        <div className="flex flex-col items-center gap-3 pt-16 pb-16 lg:min-h-180 lg:pt-16 lg:pb-0">
          <div className="mx-auto flex max-w-215 flex-col items-center gap-3 text-center">
            ...
            <h1 className="text-display text-encre max-w-186.5">
              {t.rich("title", {
                /*
            ...
            <p className="text-body text-encre/62 max-w-178">{t("lead")}</p>
