...
import { HeroOrnaments } from "@/components/sections/recouvrement/HeroOrnaments";
import { ConsultButton } from "@/components/consultation/ConsultButton";
import { Button } from "@/components/ui/Button";
import { Container } from "@/components/ui/Container";
import { MaybeLink } from "@/components/ui/MaybeLink";
import { cn } from "@/lib/utils";
import { heroStats } from "@/lib/recouvrement";
...
export function Hero() {
  const t = useTranslations("RecouvrementPage.hero");
            ...
            <h1 className="text-display text-encre relative z-0">
              {t.rich("title", {
                /*
                  ...
                  draws this bar at 605x22 at stage (660, 292) — on line TWO of
                  a three-line title, under `amiable d'abord, sans`. Against the
                  68px display size that is 8.897em wide and 0.3235em tall.

                  **Its `bottom` is the render-matched value, not the box
                  arithmetic** — see the marker note in CLAUDE.md: Figma draws
                  the glyphs 23px higher inside the same box than the browser
                  does, so 0.3418em from the frame becomes 0.0036em here.

                  **It keeps the z-index dance** even though the chunk closes the
                  title: at 605 the bar is far wider than `faiblesse ensuite.`
                  itself, so putting it behind every glyph is what Figma draws.
                ...
                */
                hl: (chunks) => (
                  <span className="relative inline-block">
                    <span
                      aria-hidden="true"
                      className="bg-pale-gold absolute bottom-[0.0036em] left-1/2 -z-10 h-[0.3235em] w-[8.897em] -translate-x-1/2 rounded-[4px]"
                    />
                    <span>{chunks}</span>
                  </span>
                ),
